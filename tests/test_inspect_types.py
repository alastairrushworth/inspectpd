...
def test_inspect_types_column_names():
    ...
    assert i_starwars.columns.tolist() == ["type", "cnt", "pcnt", "col_name"], (
        "Starwars inspect_types df ok"
    )
    assert i_tdf.columns.tolist() == ["type", "cnt", "pcnt", "col_name"], (
        "TDF inspect_types df ok"
    )
