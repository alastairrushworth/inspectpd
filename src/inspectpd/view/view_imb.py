...
def view_imb(df):
    ...
    x["col_name"] = pd.Categorical(
        x["col_name"], categories=x["col_name"], ordered=True
    )
    ...
    # build basic plot
    ggplt = (
        p9.ggplot(x, p9.aes(x="col_name", y="pcnt", fill="col_name"))
    ...
    if y1.shape[0] > 0:
        ggplt = ggplt + p9.geom_text(
            p9.aes(
                x="col_name", y="pcnt_print_loc_neg", label="value", fill="col_name"
            ),
            inherit_aes=False,
            data=y1,
            color="white",
    ...
    if y2.shape[0] > 0:
        ggplt = ggplt + p9.geom_text(
            p9.aes(
                x="col_name", y="pcnt_print_loc_pos", label="value", fill="col_name"
            ),
            inherit_aes=False,
            data=y2,
            color="gray",
