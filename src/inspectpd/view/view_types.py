...
def view_types(df):
    ...
    x["new_type"] = pd.Categorical(
        x["new_type"], categories=x["new_type"], ordered=True
    )
    x["cnt_print_loc_pos"] = (x.cnt.values) + (np.max(x.cnt.values)) / 70
    x["cnt_print_loc_neg"] = (x.cnt.values) - (np.max(x.cnt.values)) / 70
    # build basic plot
    ggplt = (
        p9.ggplot(x, p9.aes(x="new_type", y="cnt", fill="new_type"))
    ...
    if y1.shape[0] > 0:
        ggplt = ggplt + p9.geom_text(
            p9.aes(x="new_type", y="cnt_print_loc_neg", label="cnt", fill="type"),
            inherit_aes=False,
            data=y1,
            color="white",
            angle=90,
    ...
    if y2.shape[0] > 0:
        ggplt = ggplt + p9.geom_text(
            p9.aes(x="new_type", y="cnt_print_loc_pos", label="cnt", fill="type"),
            inherit_aes=False,
            data=y2,
            color="gray",
            angle=90,
