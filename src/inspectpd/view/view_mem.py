...
def view_mem(df):
    ...
    x["new_cols"] = pd.Categorical(
        x["new_cols"], categories=x["new_cols"], ordered=True
    )
    x["cnt_print_loc_pos"] = (x.pcnt.values) + (np.max(x.pcnt.values)) / 70
    x["cnt_print_loc_neg"] = (x.pcnt.values) - (np.max(x.pcnt.values)) / 70
    # build basic plot
    ggplt = (
        p9.ggplot(x, p9.aes(x="new_cols", y="pcnt", fill="new_cols"))
    ...
    if y1.shape[0] > 0:
        ggplt = ggplt + p9.geom_text(
            p9.aes(x="new_cols", y="cnt_print_loc_neg", label="size", fill="col_name"),
            inherit_aes=False,
            data=y1,
            color="white",
            angle=90,
    ...
    if y2.shape[0] > 0:
        ggplt = ggplt + p9.geom_text(
            p9.aes(x="new_cols", y="cnt_print_loc_pos", label="size", fill="col_name"),
            inherit_aes=False,
            data=y2,
            color="gray",
            angle=90,
